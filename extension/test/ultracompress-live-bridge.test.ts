...
describe.skipIf(!process.env.UC_TEST_BIN)("real UC bridge retrieval", () => {
  it("transforms text through the real binary, retrieves it exactly, and leaves retrieval readable", async () => {
    ...
    const fresh = [{ role: "toolResult", toolName: "bash", content: [{ type: "text", text: original }] }];
    expect(await context({ messages: structuredClone(fresh) }, ctx)).toBeUndefined();
    const transformed = await context({ messages: [...fresh, { role: "assistant", content: [{ type: "text", text: "consumed" }] }] }, ctx);
